...
import { defaultProductData, } from "./data_structures.js";
const getEl = (id) => {
...
const wordEl = getEl("word");
const readingEl = getEl("reading");
const meaningEl = getEl("meaning");
const progressEl = getEl("progress");
const favoriteBtn = getEl("favoriteBtn");
const easyBtn = getEl("easyBtn");
...
const easyListEl = getEl("easyList");
const closePanel = getEl("closePanel");
const easyTabBtn = getEl("easyTab");
const favoriteTabBtn = getEl("favoriteTab");
const rememberedTabBtn = getEl("rememberedTab");
const counterEl = getEl("counter");
const installBannerEl = getEl("installBanner");
const installBtn = getEl("installBtn");
const installCloseBtn = getEl("installClose");
const GROUP_SIZE = 10;
...
let cardMode = defaultProductData.cardMode;
let cardRevealState = { ...defaultProductData.cardRevealState };
let activeCardWordId = null;
let studiedCount = Number(getCookie("studiedCount") || 0);
const getRememberedCount = () => new Set([...easyWords, ...rememberedWords]).size;
...
const getGroupWords = (targetIndex) => words.slice(targetIndex * GROUP_SIZE, targetIndex * GROUP_SIZE + GROUP_SIZE);
...
const rebuildGroupData = () => {
    groupWords = getGroupWords(groupIndex).filter((word) => !easyWords.has(word.id) && !rememberedWords.has(word.id));
...
const syncStorage = () => {
    ...
    setCookie("learningState", JSON.stringify({ groupIndex, scheduleIndex, appearanceCounts }));
};
const updateCounter = () => {
    if (!counterEl)
        return;
...
const restoreToActive = (wordId, type) => {
    if (type === "easy") {
...
const render = () => {
    ...
    applyCardReveal(word);
    const positionInGroup = groupWords.findIndex((item) => item.id === word.id) + 1;
